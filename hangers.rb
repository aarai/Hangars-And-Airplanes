class Hanger
	...
	def initialize(name_of_hanger,location,owner)
		@name_of_hanger = name_of_hanger
		@location = location
		@capacity = 25
		@airplanes = []
		@owner = owner
	...
	def add_airplane(plane)
		if plane.class == Airplane
			if @capacity == 0
				"Looks like #{@name_of_hanger} hanger is full!"
			else
				@airplanes << plane
				@capacity = @capacity - 1
	...
	def self.airplanes_owned_by_hanger_owner owner
		 @owner = owner
